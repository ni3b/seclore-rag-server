...
export const HealthCheckBanner = () => {
  ...
  // Don't show expiration popup on login or signup pages
  ...
  // Check session expiration less frequently
  useEffect(() => {
    const checkSession = async () => {
      try {
        const response = await fetch('/api/me');
        if (!response.ok) {
          // Only set expired if we get a 401 or 403 and we're not on auth pages
          if ((response.status === 401 || response.status === 403) && !isAuthPage) {
            console.log("Session expired. Setting expired state to true.");
            setExpired(true);
          }
        }
      } catch (error) {
  ...
  // Don't show anything if we're on auth pages
  if (isAuthPage) {
    return null;
  }
  ...
  if (error instanceof RedirectError || expired) {
    console.log("session expired", error, expired);
    return (
      ...
      >
        <div className="flex flex-col gap-y-4">
          <p className="text-sm">
            ...
            <button
              onClick={handleLogin}
              className="w-full mt-4 mx-auto rounded-md text-text-200 py-2 bg-background-900 text-center hover:bg-emphasis animate duration-300 transition-bg"
          >
            Log in
            </button>
        </div>
    ...
    return (
      <div className="fixed top-0 left-0 z-[101] w-full text-xs mx-auto bg-gradient-to-r from-red-900 to-red-700 p-2 rounded-sm border-hidden text-text-200">
        ...
        <p className="px-1">
          If this is your initial setup or you just updated your Seclore
