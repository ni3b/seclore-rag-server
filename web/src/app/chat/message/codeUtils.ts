...
export function stripNumericOnlyLinks(html: string): string {
  return html
    // Remove <a> tags that have only numbers inside
    .replace(/<a[^>]*>\s*\d+\s*<\/a>/g, '') // <a href="...">10</a>

    // Remove links like <a>[[10]]</a>
    .replace(/<a[^>]*>\s*\[\d+\]\s*<\/a>/g, '')

    // Optional: Remove orphan numeric spans or brackets if needed
    .replace(/\[\d+\]/g, '');
}
