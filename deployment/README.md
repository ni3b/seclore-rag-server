...
The two options provided here are the easiest ways to get Seclore up and running.
...
All the features of Seclore are fully available regardless of the deployment option.

For information on setting up connectors, check out https://docs.onyx.app/connectors/overview

## Docker Compose:

Docker Compose provides the easiest way to get Seclore up and running.
...
1. To run Seclore, navigate to `docker_compose` directory and run the following:
...
3. To completely remove Seclore run:
...
1. To run Seclore, navigate to `kubernetes` directory and run the following:

   - `kubectl apply -f .`

2. To remove Seclore, run:
   - **WARNING, this will also erase your indexed data and users**
   - `kubectl delete -f .`
