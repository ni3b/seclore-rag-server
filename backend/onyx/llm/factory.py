...
def get_llms_for_persona(
    ...
    if not llm_provider:
        # Instead of failing, log a warning and fall back to default LLMs
        logger.warning(
            f"LLM provider '{provider_name}' not found for persona '{getattr(persona, 'name', 'unknown')}' "
            f"(ID: {getattr(persona, 'id', 'unknown')}). Falling back to default LLM provider."
        )
        return get_default_llms(
            temperature=temperature_override or GEN_AI_TEMPERATURE,
            additional_headers=additional_headers,
