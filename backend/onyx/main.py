...
from onyx.auth.microsoft_oidc import setup_microsoft_oidc, create_oidc_router
...
from onyx.server.manage.platform_emails import router as platform_emails_router
...
def get_application() -> FastAPI:
    application = FastAPI(title="Seclore Backend", version=__version__, lifespan=lifespan)
    if SENTRY_DSN:
        sentry_sdk.init(
            dsn=SENTRY_DSN,
    ...
    include_router_with_global_prefix_prepended(application, user_router)
    include_router_with_global_prefix_prepended(application, platform_emails_router)
    ...
    if AUTH_TYPE == AuthType.OIDC:
        # Setup Microsoft OIDC
        oauth = setup_microsoft_oidc(application)
        
        # Create and include OIDC router
        oidc_router = create_oidc_router(oauth)
        
        # Mark OIDC routes as public
        for route in oidc_router.routes:
    ...
    # Add CORS middleware first (this will run last due to middleware reverse order)
    ...
    # Session middleware is no longer needed for OIDC - OAuth library handles state internally
...
if __name__ == "__main__":
    logger.notice(
        f"Starting Seclore Backend version {__version__} on http://{APP_HOST}:{str(APP_PORT)}/"
    )
